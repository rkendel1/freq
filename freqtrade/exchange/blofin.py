...
class Blofin(Exchange):
    ...
    def _ccxt_fetch_funding_history(self, symbol=None, since=None, limit=None, params={}):
        """
        Wrapper method to make fetchFundingHistory compatible with ccxt calls.
        Returns empty list since BloFin doesn't support this properly.
        """
        logger.info("🔧 _ccxt_fetch_funding_history called for %s, returning empty list", symbol)
        return []
    ...
    def get_funding_fees(
        ...
        For BloFin, we'll return 0.0 for now to avoid errors and allow profit calculations.
        """
        logger.info("🔧 get_funding_fees called for %s, amount: %s, is_short: %s", pair, amount, is_short)
        
        # For now, return 0.0 to allow profit calculations to work
        # TODO: Implement proper funding fee calculation when BloFin API supports it
        logger.info("🔧 Returning 0.0 funding fees for %s (BloFin implementation pending)", pair)
        return 0.0
