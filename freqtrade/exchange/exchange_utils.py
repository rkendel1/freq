...
def validate_exchange(exchange: str) -> tuple[bool, str, str, ccxt.Exchange | None]:
    ...
    if exchange.lower() == "blofin":
        # Skip fetchOrder check for BloFin since we implement it ourselves
        blofin_required = {k: v for k, v in EXCHANGE_HAS_REQUIRED.items() if k != "fetchOrder"}
        missing = _exchange_has_helper(ex_mod, blofin_required)
    else:
