...
class CatboostClassifierMultiTargetLopezDePrado(BaseClassifierModel):
    """
    ...
    def fit(self, data_dictionary: dict, dk: FreqaiDataKitchen, **kwargs) -> Any:
        ...
        if not use_purged_cv or n_splits < 3:
            logger.info("Training multi-target CatBoost (purged CV disabled or n_splits < 3)")
            return self._train_multi_target_single(X, y, sample_weight, data_dictionary, dk)

        logger.info(
            f"Training multi-target CatBoost with Lopez de Prado: {n_splits} folds, "
            f"{y.shape[1]} targets"
        )
...
class MultiTargetEnsembleWrapper:
    """Wrapper for multi-target ensemble predictions."""
