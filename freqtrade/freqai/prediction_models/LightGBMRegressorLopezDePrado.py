...
class LightGBMRegressorLopezDePrado(BaseRegressionModel):
    """
    ...
    def fit(self, data_dictionary: dict, dk: FreqaiDataKitchen, **kwargs) -> Any:
        ...
        if not use_purged_cv or n_splits < 3:
            logger.info("Training single LightGBM regressor (purged CV disabled or n_splits < 3)")
            return self._train_single_model(X, y, sample_weights, data_dictionary, dk)

        logger.info(
            f"Training Lopez de Prado LightGBM regressor ensemble: {n_splits} purged folds, "
        ...
        logger.info(
            f"LightGBM regressor ensemble complete: {len(models)} models, "
