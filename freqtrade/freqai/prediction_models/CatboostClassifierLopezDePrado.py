...
class CatboostClassifierLopezDePrado(BaseClassifierModel):
    """
    ...
    def fit(self, data_dictionary: dict, dk: FreqaiDataKitchen, **kwargs) -> Any:
        ...
        if not use_purged_cv or n_splits < 3:
            logger.info("Training single CatBoost model (purged CV disabled or n_splits < 3)")
            return self._train_single_model(X, y, sample_weights, data_dictionary, dk)

        logger.info(
            f"Training Lopez de Prado CatBoost ensemble: {n_splits} purged folds, "
        ...
        logger.info(
            f"CatBoost ensemble complete: {len(models)} models, "
